...
namespace cm {
    ...
    inline HalconCpp::HObject MatToHObject(const cv::Mat& image) {
        ...
        if (image.type() == CV_8UC1) {
            ...
            HalconCpp::GenImage3(&hObj, "byte", width, height,
                (Hlong)(planes[2].data), // Red plane
                (Hlong)(planes[1].data), // Green plane
                (Hlong)(planes[0].data)); // Blue plane
        }
    ...
    inline void VisualizeHObjectWin(HalconCpp::HObject inHObj, cv::Size inSize) {
        ...
        HalconCpp::OpenWindow(0, 0, inSize.width, inSize.height, 0, "visible", "", &hv_WindowHandle);

        HalconCpp::SetWindowParam(hv_WindowHandle, "background_color", "black");

        HalconCpp::ClearWindow(hv_WindowHandle);

        HalconCpp::SetColor(hv_WindowHandle, "green");
        HalconCpp::SetLineWidth(hv_WindowHandle, 2);
        HalconCpp::SetDraw(hv_WindowHandle, "margin");

        HalconCpp::DispObj(inHObj, hv_WindowHandle);
